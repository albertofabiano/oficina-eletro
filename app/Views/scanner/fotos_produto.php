<div class="card">
  ...
  <p class="hint">
    Tire até 4 fotos do produto (a primeira vira a capa, as demais entram na galeria).
    Elas ficam anexadas ao cadastro no computador.
  </p>
...
(function () {
  ...
  function render() {
    box.innerHTML = fotos.map(function (d, i) {
      return '<div style="position:relative">' +
        ...
        (i === 0 ? '<span style="position:absolute;bottom:-6px;left:2px;background:#0d6efd;color:#fff;border-radius:6px;font-size:10px;padding:1px 5px">Capa</span>' : '') +
  ...
  btn.addEventListener('click', async function () {
    ...
    try {
      var r = await fetch('<?= url('/scan/' . $token . '/fotos-produto') ?>', {
        method: 'POST',
