<?php
/**
 * Torna "Aprovado" (orçamento aprovado pelo cliente, tipo Aberta) nativo (bloqueado=1,
 * codigo='aprovado') pra TODAS as empresas já cadastradas — LandingController::registrar() já
 * semeia esse status pra empresa NOVA a partir de agora (ver CLAUDE.md "Botão primário da OS
 * 'Aguardando aprovação'..." / "Checkbox 'Exibir botão Fechar OS'..."), este script cobre o
 * passado, mesmo padrão já usado pra "Não Apresenta Defeito" (tornar_nativo_status_sem_defeito.php).
 *
 ...
 *   - Já tem um status com nome batendo "aprovado" (ou já codigo='aprovado', reentrância) → só
 *     marca esse MESMO registro como nativo (codigo='aprovado', bloqueado=1). Não mexe em
 *     cor/tipo/permite_fechar/sem_valor — preserva o que a empresa já configurou, só trava o
 *     registro como nativo.
 ...
 *   php scripts/tornar_nativo_status_aprovado.php --aplicar
 */
...
$stmtBusca = $db->prepare(
    "SELECT id, nome, codigo FROM os_status WHERE empresa_id = ?
     AND (codigo = 'aprovado' OR nome LIKE '%aprovad%')
     ORDER BY id"
);
...
foreach ($empresasComOs as $eid) {
    ...
    foreach ($candidatos as $c) {
        ...
        if ($c['codigo'] === 'aprovado' || str_contains($norm, 'aprovad')) {
            $achado = $c;
            break;
        }
    }
    ...
    if ($achado) {
        if ($achado['codigo'] === 'aprovado') { $jaNativo++; continue; } // já rodado antes
...
$stmtUpd = $db->prepare("UPDATE os_status SET codigo = 'aprovado', bloqueado = 1 WHERE id = ?");
$idsMarcados = [];
...
$stmtIns = $db->prepare(
    "INSERT INTO os_status (empresa_id, codigo, nome, cor, cor_fonte, ordem, tipo, permite_fechar, sem_valor, bloqueado)
     VALUES (?, 'aprovado', 'Aprovado', '#20c997', '#ffffff', ?, 'aberta', 1, 0, 1)"
);
$idsCriados = [];
